...
def _sheet_phased(wb, ph):
    ...
    n_al = sum(1 for st in ph.stages if st.aligned_milestones)
    if ph.milestone_mode == "すべて":
        r = _note(ws, r, "マイルストーン: 記入済みの日付をすべて使用(「予測」シートと同じ条件)。"
                         "実運用では着手前から予定が引かれているため運用の実態に近いが、"
                         "完了案件では記入値が実績の日付なので、予定が完璧に当たった場合"
                         "= 精度の上限にあたる。settings の 段階予測のマイルストーン を "
                         "通過済み にすると、未来を覗かない厳しい条件(= 下限)で比較できる。")
    else:
        r = _note(ws, r, "マイルストーン: その段階までに通過したものだけを使用。"
                         "未来を一切覗かないぶん厳密だが、予定を何も持たない場合"
                         "= 精度の下限にあたる。背骨のマイルストーンを通過していない段階では"
                         "位置合わせが効かず、素朴版と同じカーブになる。")
    r = _note(ws, r, f"位置合わせが効いた段階: {n_al}/{len(ph.stages)}"
                     "(効いていない段階は素朴版と同じ形。誤差表の「位置合わせに使ったMS数」を参照)")
    if ph.ramp_limit:
        r = _note(ws, r, f"立ち上がり上限: 前月比 {ph.ramp_limit:.2f} 倍/月"
                         "(実績の前月比の分布から決めた値)。"
                         "人員は急には増やせないため、確定区間の直後に工数が跳ねないよう"
                         "増加率に上限を置き、あふれた工数を後ろの月へ送っている。"
                         "総工数と行程グループ別の総量は動かしていない。")
    else:
        r = _note(ws, r, "立ち上がり上限: なし。確定区間の直後に、現実には出せない"
                         "急な立ち上がりが出ることがある。")
    ...
    r = _note(ws, r, "「見込んだ総工数」は どの段階でも同じ値になる。"
                     "総工数は契約人月(または見積もり)で決まっており、"
                     "実績を確定させても動かないため。確定分が既にそれを超えている段階だけ、"
                     "実績そのものが下限になって大きくなる。")
    r = _note(ws, r, "「立上り制約に当たった月数」が 0 でない段階は、学習カーブどおりに"
                     "配ると立ち上がりが急すぎるため、工数を後ろの月へ送っている。"
                     "その段階では山が後ろへ移動し、なだらかになる。")
    r = _note(ws, r, "「残工数の倍率」は 残工数 ÷ 学習カーブが残り区間に置く量。"
                     "1.0 なら学習カーブどおり。1 より大きいほど確定分が学習カーブより少なく、"
                     "そのずれが残り区間へ上乗せされる。切り替え点の段差の高さは この倍率そのもの。")
    ...
    r_next, mhead, mlast = _table(
        ...
        col_width={"段階": 6, "区切り": 16, "確定した月": 12, "確定月数": 9,
                   "見込んだ総工数(時間)": 18, "残工数の倍率(学習カーブ比)": 18,
                   "立上り制約に当たった月数": 18, "位置合わせに使ったMS数": 16,
                   "残り月数": 9, "評価した月数": 11,
                   "残り実績(時間)": 15,
