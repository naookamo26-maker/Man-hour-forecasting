...
def forecast(model: Model, ds, pid: str, *,
             ...
             months_override: list[str] | None = None) -> Forecast:
    ...
    if not np.isfinite(mm) or mm <= 0:
        raise ValueError(
            ...
            "予測対象の総工数を決められないため、projects シートの 契約人月 を入力してください。")
    total_hours = mm * model.hours_per_mm

    if group_totals:
        # 見積もり分類の値が与えられている場合はそれを正とする(設計書 6 Step1)
        gt = pd.Series(group_totals).reindex(model.groups).fillna(0.0)
        if gt.sum() <= 0:
            raise ValueError("見積もり分類の合計が 0 です。")
        gt = gt / gt.sum() * total_hours
        notes.append("行程グループ間の総量は見積もり分類の指定値で固定しています。")
    else:
        gt = model.group_ratio.reindex(model.groups).fillna(0.0) * total_hours
    ...
    table = pd.DataFrame(cols, index=pd.Index(months, name="月"))[model.groups]

    # 丸め誤差の吸収。合計は必ず総時間に一致させる。
    diff = total_hours - table.to_numpy().sum()
    if abs(diff) > 1e-6:
        biggest = table.sum(axis=1).idxmax()
        table.loc[biggest, model.groups[int(np.argmax(gt.to_numpy()))]] += diff
